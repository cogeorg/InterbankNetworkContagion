...
#-------------------------------------------------------------------------
	...
	def bank__liquidate_assets(self, args):
		...
		text += " \n"
		text += "  Under the current circumstances this will lead to Lp = -160.0 in the first loop \n"
	...
	def bank__initialize_transactions(self, args):
		...
		text += "  This methode is supposed to create the transactions of banks in BlackRihno. \n"
		text += "  The simplest way to test it is to initialize the standard bank, then to call the \n"
		text += "  mehtode and check whether the standard bank has been overwritten. \n"
		text += "   \n"
		self.print_info(text)		
		...
		print bank
		state = State()
		environment.get_state(0)
		state.print_state()
		#bank.get_parameters_from_file(bankFilename, state,  3.0,  0)
