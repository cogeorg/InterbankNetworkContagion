...
#
	...
	def __str__(self):
		...
		text += "    <value name='Ip' value='" + str(self.Ip) + "'></value>\n"
		text += "    <value name='Ep' value='" + str(self.Ep) + "'></value>\n"
		text += "</bank>\n"
...
#
	...
	def get_central_bank_liquidity(self,  state):
		...
		if (self.Lp < 0.0):
			# remove central bank transaction from stack
			old_LC = self.get_account("LC")
			...
			for transaction in self.accounts:
				if (transaction.transactionType == "LC"):
					transaction.transactionValue = old_LC + value
	...
	def liquidate_assets(self,  initial_assets,  current_assets,  state,  debug,  time):
		...
		if (self.Lp < 0.0): # we have a shortfall in liquidity
			...
			if (self.Ip > current_I): # only if planned exceeds current we can reduce planned
				self.Ip = current_I # this is crap
				self.Lp += (self.Ip - current_I)
			if (self.Ep > current_E):
				self.Ep = current_E
				self.Lp += (self.Ep - current_E)
			
			print self.Ip,  self.Lp,  self.Ep
